...
app.get('/carteraClientes',verificarSesion, (req, res)=>{     
  connection.query('SELECT * FROM clientes WHERE tipoCliente != ?',['EDIFICIOS ADMINISTRACION'],(error, results)=>{
      if(error){
          throw error;
      } else {
        connection.query('SELECT * FROM clientes WHERE tipoCliente = ?',['EDIFICIOS ADMINISTRACION'],(error, results2)=>{
          if(error){
              throw error;
          } else {
              const datos = req.session.datos;                     
...
app.get('/createClientAdministration', (req,res)=>{
      ...
      if(error){
        ...
        connection.query('SELECT MAX(consec) + 1 AS consec FROM folios ',(error,results2)=>{
          if(error){
            ...
            const opcionesBloque = results.map(row => row.nombreBloque ); 
            const datos = req.session.datos;
          res.render('createClientSuministro',{opcionesBloque,client:results2[0],datos});
          console.log(opcionesBloque);
          }
        })
